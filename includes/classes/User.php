...
//La POO aide à garder le code PHP SEC "Ne vous répétez pas" et rend le code plus facile à maintenir, modifier et déboguer.
// $this = objet courant 
class User {
	private $user;
	private $user2;
	...
	public function isStudent($username_to_check) {
		...
		if(isset($this->user2['student_array']) && (strstr($this->user2['student_array'], $usernameComma) || $username_to_check == $this->user2['username'])) {
			return true;
		}
		elseif(isset($this->user2['student_array']) && (strstr($this->code['student_array'], $usernameComma))){
            return true;
		}
		else {
