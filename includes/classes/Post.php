 <?php
	class Post
	{
		...
		public function submitPost($body, $fileName, $fileDestination,$user_to)//teacher
		{
			...
			if ($check_empty != "" && $fileName == "") {
				...
				$joined = $this->user_obj->getMembers();
                ...
                if($user_to != 'none') {
					...
					$notification->insertNotification($returned_id, $user_to, "classRoom_post", $this->code);
				} 
			...
			if ($fileName != "") {  //only assignment
                ...
                if($user_to != 'none') {
					...
					$notification = new  User2($this->con, $added_by);
					$joined_qry = mysqli_query($this->con, "SELECT `student_array` FROM `createclass` where `courseCode` = '{$this->code}' ");
					if(mysqli_num_rows($joined_qry) > 0){
						$join_row = mysqli_fetch_array($joined_qry);
						$members = explode(" ,", $join_row['student_array']);
						if(count($members) > 0){
								foreach($members as $member){
									if(!empty($member)){
										$notification->insertNotification($returned_id, $member, "classRoom_post", $this->code);
									}
								}
						}
					}
...
$start_date = new DateTime($date_time); // Assuming $date_time is defined elsewhere, Time of post
$end_date = new DateTime($date_time_now); //Current time
$interval = $start_date->diff($end_date); //Difference between dates 

if ($interval->y >= 1) {
    // Fixed comparison here for 1 year check
    $time_message = $interval->y . " year" . ($interval->y > 1 ? "s" : "") . " ago";
} else if ($interval->m >= 1) {
    $days = ""; // Initialize days string
    if ($interval->d == 0) {
        $days = " ago";
    } else if ($interval->d == 1) {
        $days = " and " . $interval->d . " day ago";
    } else {
	...
	$str .= "<div class='status_post' data-id='$id' >
										...
										<div id='post_body'>
										 ...
										 <div class='commentOption' id='comment$id' onClick='javascript:toggle$id()'> 
										   Comments($comments_check_num)<span class='edited-det'>$checkEdit  </span> 
										 </div>
								       </div>
								          <div class='post_comment' id='toggleComment$id' style='display:none;'>
											<iframe src='comment_frame.php?post_id=$id' class='comment_iframe'  data-id='$id' frameborder='0' ></iframe>
...
if ($years >= 1) {
    $time_message = $years . " year" . ($years > 1 ? "s" : "") . " ago";
} else if ($months >= 1) {
    $days_str = ($days == 0) ? " ago" : 
                ($days == 1 ? " and 1 day ago" : " and {$days} days ago");
    $time_message = $months . " month" . ($months > 1 ? "s" : "") . $days_str;
} else if ($days >= 1) {
    $time_message = ($days == 1) ? "Yesterday" : $days . " days ago";
} else if ($hours >= 1) {
    $time_message = $hours . " hour" . ($hours > 1 ? "s" : "") . " ago";
} else if ($minutes >= 1) {
    $time_message = $minutes . " minute" . ($minutes > 1 ? "s" : "") . " ago";
} else {
    $time_message = ($seconds < 30) ? "Just now" : $seconds . " seconds ago";
}
...
public function loadTeachingClasses()
{
	...
	if (mysqli_num_rows($data_query) > 0) {
		while ($row = mysqli_fetch_array($data_query)) {
			...
			if ($userLoggedIn == $added_by) {
				$delete_teachingClass = "<a class='delete-class-btn' href='includes/form_handlers/delete_post.php?createClass_id=$id&courseCode=$code'><input type='button' id='delete_class_btn' value='Remove'></a>";
			} else {
...
public function loadEnrolledClasses()
{
	...
	if (mysqli_num_rows($data_query) > 0) {
		while ($row = mysqli_fetch_array($data_query)) {
			...
			$delete_EnrolledClass = "<a class='delete-class-btn' href='includes/form_handlers/delete_post.php?Enrolled_Student=$this->user&amp;classCode=$code'><input type='button' id='delete_class_btn' value='Leave'></a>";
