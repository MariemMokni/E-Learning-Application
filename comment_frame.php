    ...
    <div class="comment_wrapper">
		...
		if (isset($_POST['postComment' . $post_id])) {
			...
			if($posted_to != $userLoggedIn) {
				...
				$notification->insertNotification($post_id, $posted_to, "comment", $courseCode, $comment_id);
			}
			
			if($user_to != 'none' && $user_to != $userLoggedIn) {
				$notification = new User2($con, $userLoggedIn);
				$notification->insertNotification($post_id, $user_to, "classRoom_comment", $courseCode, $comment_id);
			}
			...
			while($row = mysqli_fetch_array($get_commenters)) {
				...
				if($row['posted_by'] != $posted_to && $row['posted_by'] != $user_to 
					...
					$notification = new User2($con, $userLoggedIn);
					$notification->insertNotification($post_id, $row['posted_by'], "comment_non_owner", $courseCode, $comment_id);
...
if ($interval->y >= 1) {
    ...
    if ($interval->s < 30) {
        ...
        $time_message = $interval->s . " seconds ago";
				...
				?>
        <div class="comment_section" data-id="<?= $id ?>">
